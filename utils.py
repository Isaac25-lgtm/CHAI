...
class ExcelGenerator:
    ...
    def create_assessment_excel(assessment_data: Dict, protect_sheet: bool = False) -> Tuple[str, str]:
        ...
        try:
            ...
            # Apply protection to all sheets (always for section downloads, optional for full reports)
            if protect_sheet or is_section_download:
                for ws in wb.worksheets:
                    ws.protection.sheet = True
                    ws.protection.password = 'palace111'
                    ws.protection.enable()
                    # Add watermark
                    ws['Z1'] = 'READ ONLY - Contact superuser for editing'
                    ws['Z1'].font = Font(color="FF0000", size=8)
    ...
    def _create_summary_sheet(wb: Workbook, assessment_data: Dict):
        ...
        if is_section_download:
            ...
            section_key = sections_with_data[0]
            all_question_keys = [k for k in scores.keys() if k.startswith(section_key) or k == section_key]
            
            # Debug: Print what we're working with (force flush to see output immediately)
            import sys
            print(f"\n{'='*60}", flush=True)
            print(f"DEBUG: Section key: {section_key}", flush=True)
            print(f"DEBUG: Question keys found: {all_question_keys}", flush=True)
            print(f"DEBUG: Total scores in data: {len(scores)}", flush=True)
            
            # Build question text map from section definitions
            question_text_map = {}
            section_definitions = assessment_data.get('section_definitions', {})
            print(f"DEBUG: Section definitions available: {len(section_definitions)} sections", flush=True)
            if section_key in section_definitions:
                print(f"DEBUG: Found config for {section_key}", flush=True)
            else:
                print(f"DEBUG: NO CONFIG for {section_key}!", flush=True)
            ...
            print(f"DEBUG: Sample mappings: {dict(list(question_text_map.items())[:3])}", flush=True)
